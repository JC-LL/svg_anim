...
film.add text2=SVG::Text.new(500,500,"easy drawing!",:"font-size" => 20,fill:"greenyellow")
...
180.times do |t|
  film.frame(t) do |t|
    text1.rotate(t*2) #degres !
    text2.rotate(360-t*2) #degres !
    rect.move_by 4,4
    rect.rotate t*10
    rad=(PI/180)*t/2
    circles_out.each_with_index{|c,i|
      x=CENTER.x+RAYON_1*cos((2*PI/NB_CIRCLES_OUT)*i+rad)
      y=CENTER.y+RAYON_1*sin((2*PI/NB_CIRCLES_OUT)*i+rad)
      c.move_to(x,y)
    }
    rad*=8
